...
extern uint8_t err_flag;									// zmienna globalna wykorzystywana
															// w bibliotece TWI do kontroli po³¹czenia
uint8_t BMP180_init( void ) 
{
        uint8_t bufor[22];									// utworzenie zmiennej na odczytane dane
        TWI_read_buf( BMP180_ADDR, 0xAA, 22, buf );			// odczyt zawartoœci pamiêci BMP180
		...
		return err_flag;									// kontrola po³¹czenia z magistrali TWI
