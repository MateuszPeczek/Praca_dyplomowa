...
void _LCD_OutNibble(unsigned char nibbleToWrite) // Wystawienie pÛ≥bajtu na magistralÍ danych
void _LCD_Write(unsigned char dataToWrite)	// Zapis bajtu danych do wyúwietlacza
void LCD_WriteCommand(unsigned char);	// Zapis rozkazu sterujπcego wyúwietlaczem
void LCD_WriteData(unsigned char);	// Zapis danych do wyúwietlacza
void LCD_WriteText(char *);	// Wyúwietlenia napisu na wyúwietlaczu
void LCD_GoTo(unsigned char, unsigned char);	// Ustawienie wspÛ≥rzÍdnych ekranu 
void LCD_Clear(void);	// Wyczyszczenie wyúwietlacza
void LCD_Home(void);	// PrzywrÛcenie domyúlnych wartoúci wyúwietlacza
void LCD_Initalize(void);	// Inicjalizacja wyúwietlacza
